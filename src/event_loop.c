...
static void *event_loop_thread(void *arg)
{
	...
	while (1) {
		...
		while ((task = event_loop_get_next_task_to_run(el)) == NULL) {
			...
			/*
               We are here when event loop thread receives signal.
               On receiving the signal, go back, fetch the new task
               from the task array again and decide of we want to block
               again or fire the task
            */
		}

		el->ev_loop_state = EV_LOOP_BUSY;
		/*
           We are done with the task array, unlock it now, object of
           interest is already detached from task array
        */
...
static void event_loop_remove_task_from_task_array(event_loop_t *el,
						   task_t *task)
{
